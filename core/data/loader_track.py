...
class DriverData(torch.utils.data.Dataset):
    ...
    def _load_one_record(self, frame_key):
        if not hasattr(self, '_lmdb_engine'):
            self._init_lmdb_database()
        if self._randomize:
            random_frame_key = random.choice([frame_id for frame_id in self._frames if frame_id != frame_key])
            random_f_image = torchvision.transforms.functional.resize(
                self._lmdb_engine[random_frame_key].float() / 255.0, (518, 518), antialias=True)
            # print(type(self._data[random_frame_key]['transform_matrix']))
            if not torch.is_tensor(self._data[random_frame_key]['transform_matrix']):
                random_f_transform = torch.from_numpy(self._data[random_frame_key]['transform_matrix']).float().cpu()
            else:
                random_f_transform = self._data[random_frame_key]['transform_matrix'].float().cpu()
            random_f_planes = build_points_planes(self.point_plane_size, random_f_transform)
            # random_f_shape = torch.tensor(self._data[random_frame_key]['shapecode']).float().cpu()
        ...
        if self._randomize:
            one_data = {
                'f_image': random_f_image, 'f_planes': random_f_planes,
                't_image': t_image, 't_points': t_points, 't_transform': this_record['transform_matrix'], 't_bbox': this_record['bbox'],
