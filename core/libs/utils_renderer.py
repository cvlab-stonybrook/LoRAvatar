...
NUM_CHANNELS = 32
NUM_CHANNELS_512D = 2048
def render_gaussian_finetune(gs_params, cam_matrix, cam_params=None, sh_degree=0, bg_color=None):
    # Build params
    ...
    points, colors, opacities, scales, rotations = \
        gs_params.points, gs_params.colors, gs_params.opacities, gs_params.scales, gs_params.rotations
...
# def render_gaussian_512d(gs_params, cam_matrix, cam_params=None, sh_degree=0, bg_color=None):
#     # Build params
#     batch_size = cam_matrix.shape[0]
#     focal_x, focal_y, cam_size = cam_params['focal_x'], cam_params['focal_y'], cam_params['size']
#     points, colors, opacities, scales, rotations = \
...
#     view_mat, proj_mat, cam_pos = build_camera_matrices(cam_matrix, focal_x, focal_y)
#     bg_color = cam_matrix.new_zeros(batch_size, NUM_CHANNELS_512D, dtype=torch.float32) if bg_color is None else bg_color
#     # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
#     means2D = torch.zeros_like(points, dtype=points.dtype, requires_grad=True, device="cuda") + 0
#     try:
#         means2D.retain_grad()
