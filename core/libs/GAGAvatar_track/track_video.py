...
class Tracker:
    ...
    def track_video(self, video_path, dir_path=None, no_vis=False, data_name=None):
        ...
        # self.run_visibility(optim_results, lmdb_engine, output_path, fps=fps)
        lmdb_engine.close()
    
    def run_visibility(self, data_result, lmdb_engine, output_path, fps):
        import torchvision
        ...
        mesh_render = RenderMesh(
            1024, faces=self.flame_model.get_faces().cpu().numpy(), device=self._device, visible_verts=True,
        )
        ...
        for frame in tqdm(frames, ncols=80, colour='#95bb72'):
            ...
            images, alpha_images, visible_vertices = mesh_render(
            ...
            print("visible_vertices indices", visible_vertices, visible_vertices.size())
...
def run_smoothing(lightning_result, output_path):
    ...
    smoothed_results = {}
    expression, pose, eyecode, rotates, translates, lmks_2d = [], [], [], [], [], []
    ...
    for frame_name in frames:
        ...
        lmks_2d.append(smoothed_results[frame_name]['lmks_2d'])
    ...
    lmks_2d = smooth_params(np.stack(lmks_2d), alpha=0.7)
    ...
    for fidx, frame_name in enumerate(frames):
        ...
        smoothed_results[frame_name]['lmks_2d'] = lmks_2d[fidx]
