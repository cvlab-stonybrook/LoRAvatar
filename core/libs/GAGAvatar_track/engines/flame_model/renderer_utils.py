...
class RenderMesh(nn.Module):
    def __init__(self, image_size, obj_filename=None, faces=None, device='cpu', visible_verts=False):
        super(RenderMesh, self).__init__()
        self.device = device
        self.image_size = image_size
        self.visible_verts = visible_verts
        ...
        self.raster_settings = RasterizationSettings(image_size=image_size, blur_radius=0.0, faces_per_pixel=10)
    ...
    def forward(
        ...
        if self.visible_verts:
            vertices_n = vertices.squeeze().cpu().numpy()
            faces_n = faces.squeeze().cpu().numpy()
            transform_matrix_n = transform_matrix.numpy()
            tmesh = trimesh.Trimesh(vertices=vertices_n, faces=faces_n, process=False)

            # Camera origin in world coordinates
            camera_origin = -transform_matrix_n[:3, :3].T @ transform_matrix_n[:3, 3]
            ...
            directions = vertices_n - camera_origin
            directions = directions / np.linalg.norm(directions, axis=1, keepdims=True)

            # Cast rays
            ray_intersector = trimesh.ray.ray_pyembree.RayMeshIntersector(
                tmesh) if trimesh.ray.has_embree else trimesh.ray.ray_triangle.RayMeshIntersector(tmesh)
            locations, index_ray, index_tri = ray_intersector.intersects_location(
                ray_origins=np.repeat([camera_origin], len(vertices_n), axis=0),
                ray_directions=directions,
                multiple_hits=True
            )

            # Compute distance to actual target vertices
            vertex_distances = np.linalg.norm(vertices_n - camera_origin, axis=1)

            # Find distance to intersections
            intersection_distances = np.full(len(vertices_n), np.inf)
            ...
            visible_verts = np.where(visible_mask)[0]

            # raster = MeshRasterizer(cameras=cameras, raster_settings=self.raster_settings)
            # fragments = raster(mesh)
            # visible_faces = fragments.pix_to_face[0, ..., 0]
            # visible_faces = visible_faces[visible_faces>=0].unique()
            # visible_verts = mesh.faces_packed()[visible_faces].unique()
        ...
        if self.visible_verts:
            return images*255, alpha_images, visible_verts
        else:
